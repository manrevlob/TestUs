...
class BuildsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create build" do
    assert_difference('Build.count') do
      post builds_url, params: { build: { active: @build.active, description: @build.description, isOpen: @build.isOpen, name: @build.name, releaseDate: @build.releaseDate } }
    end
  ...
  test "should update build" do
    patch build_url(@build), params: { build: { active: @build.active, description: @build.description, isOpen: @build.isOpen, name: @build.name, releaseDate: @build.releaseDate } }
